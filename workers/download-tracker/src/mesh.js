...
export const QNS_CD = Object.freeze({
  ...
  author: IDENTITY,
  identity: IDENTITY,
  note: "Hub cite / Worker mesh cross-map only. Local qnsd is qnm-node. Not a Softwares-tab product. No public qnsd proxy. Author: Aziel Eliab only.",
});
...
export const MESH_PRODUCT = "azbrowser";
...
export const MESH_NOTE =
  "QNM-BUILD-1.0 + QNS-CD-1.0. Suite mesh default off. Live|locked|isolated counts only. Photon QNS1 packet transfer is local qnm-node (no public qnsd proxy). No Node Gate. No auto-heal. Not an anonymity network. Author: Aziel Eliab only.";
...
export const MESH_PROXY_ROUTES = Object.freeze([
  ...
  { path: MESH_ENABLE_PATH, methods: ["post"], op: "enable", summary: "PROXY to aziel-runtime POST /v1/mesh/enable. Global kill switch ON. Rate-limited. Not a local op." },
  ...
  { path: MESH_JOIN_PATH, methods: ["post"], op: "join", summary: "PROXY to aziel-runtime POST /v1/mesh/join. Body {product, node_id?, label?}. Refused while OFF. Not a local op." },
...
export function parseMeshDoc(body) {
  ...
  return emptyMesh({
    ...
    note: enabled
      ? "QNM-BUILD-1.0 + QNS-CD-1.0. Suite mesh is on. Live|locked|isolated counts only. Photon QNS1 packet transfer is local qnm-node (no public qnsd proxy). No Node Gate. No auto-heal. Not an anonymity network."
      : MESH_NOTE,
  });
}
...
export function meshPointer() {
  return {
    ...
    origin: RUNTIME + MESH_PATH,
    note: "PROXY to aziel-runtime /v1/mesh/* via AZIEL_RUNTIME. Not a local op. Not AnonBroadcast. Not AZMail's product-local ring. " + MESH_NOTE,
...
export function meshOpenApiPaths() {
  ...
  for (const route of MESH_PROXY_ROUTES) {
    ...
    for (const method of route.methods) {
      entry[method] = {
        operationId: "azbrowser_mesh_" + route.op + (method === "head" ? "_head" : "") + "_proxy",
