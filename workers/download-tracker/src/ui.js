import { AZNET, AZNET_GARDEN, AZNET_WORKER, HOST, LIMITATION, SIGIL, VERSION } from "./engine.js";
...
.pair{font-size:11px;letter-spacing:.06em;text-transform:uppercase;border:1px solid var(--trim);border-radius:8px;padding:4px 8px;color:var(--gold)}
.pair.off{color:#f0d78c;border-color:#8a7219}
iframe.sidenet{width:100%;min-height:360px;border:1px solid var(--trim);background:#fff;border-radius:8px}
...
<div id="win">
  ...
  <div id="toolbar">
    ...
    <input id="omnibox" placeholder="Lamb Lens search or enter a URL" spellcheck="false" autocomplete="off">
    <button id="btnGo" type="button" title="Go / ethical search">Go</button>
    <button id="btnSidenet" type="button" title="AZNet side-net viewer">Side-net</button>
    <button id="btnPair" type="button" title="Pair status">Pair</button>
    <span id="pairBadge" class="pair off">Pair: …</span>
  ...
  <div id="body">
    ...
    <aside id="side">
      ...
      <div class="card cite">
        ...
        Sibling mail: <a href="https://github.com/AzielEliab/azmail">AZMail</a><br>
        Sibling side-net: <a href="${AZNET}">AZNet</a> ·
        <a href="${AZNET_WORKER}">Worker</a> ·
        <a href="${AZNET_GARDEN}">garden</a>
      </div>
    </aside>
  </div>
  <div id="status">
...
const AZNET = ${JSON.stringify(AZNET)};
const AZNET_WORKER = ${JSON.stringify(AZNET_WORKER)};
const AZNET_GARDEN = ${JSON.stringify(AZNET_GARDEN)};
let sessionId = "";
const clientTabs = [];
...
function setPairBadge(j) {
  const el = document.getElementById("pairBadge");
  if (!el) return;
  const paired = !!(j && (j.paired === true || (j.pair && j.pair.paired)));
  el.textContent = paired ? "Pair: ready" : "Pair: required";
  el.className = "pair" + (paired ? "" : " off");
}
function homePanel() {
  return '<div class="sigil-home"><img alt="everblooming sigil" src="'+SIGIL+'"><h1 style="color:#c9a227;font-weight:500">AZBrowser</h1><p>Lamb Lens ethical search · AZNet side-net viewer · Phase 1 research shell</p></div>'
    + '<div class="banner">'+LIMITATION+'</div>'
    + '<div class="card"><p>Both products required. Pair with sibling <a href="'+AZNET+'">AZNet</a> to run research. FragGate unlocks; StaticClock times.</p>'
    + '<p>Deep links: <a href="'+AZNET_WORKER+'">AZNet Worker</a> · <a href="'+AZNET_GARDEN+'">garden</a>. Viewer only — protocol lives in AZNet.</p></div>'
    + '<div class="card"><p>New-tab search is Lamb Lens: cite sources, refuse doxxing / credential harvest / malware lure. Advisory. AZNet branding is the Side-net button, not this search.</p>'
    + '<p>Type a query or HTTPS URL in the address bar. Home button is the everblooming sigil.</p></div>';
}
function renderResult(j) {
  const stage = document.getElementById("stage");
  if (j.paired === true || j.paired === false || (j.pair && typeof j.pair.paired === "boolean")) setPairBadge(j);
  if (j.action === "home" || (j.tab && j.tab.kind === "newtab" && !j.results && !j.html && j.code !== "PAIR_REQUIRED")) {
    stage.innerHTML = homePanel();
    return;
  }
  if (j.code === "PAIR_REQUIRED") {
    const links = (j.pair && j.pair.links) || {};
    stage.innerHTML = '<div class="banner">AZNet pairing required. Both products must be paired. FragGate unlocks; StaticClock times.</div>'
      + '<div class="card"><p>AZBrowser views the side-net. It does not embed the AZNet protocol.</p>'
      + '<p><a href="'+(links.aznet_github||AZNET)+'">AZNet GitHub</a> · <a href="'+(links.aznet_worker||AZNET_WORKER)+'">Worker</a> · <a href="'+(links.aznet_garden||AZNET_GARDEN)+'">garden</a></p></div>';
    return;
  }
  if (j.action === "sidenet_view" || j.viewer) {
    stage.innerHTML = '<div class="banner">AZNet side-net viewer — not a whitepaper fork. Protocol lives in the AZNet repo.</div>'
      + '<div class="card"><p><a href="'+(j.github||AZNET)+'">GitHub</a> · <a href="'+(j.worker||AZNET_WORKER)+'">Worker</a> · <a href="'+(j.garden||AZNET_GARDEN)+'">garden</a></p>'
      + '<p class="cite">'+(j.note||"")+'</p></div>'
      + '<iframe class="sidenet" title="AZNet garden" src="'+(j.garden||AZNET_GARDEN)+'"></iframe>';
    return;
  }
...
document.getElementById("btnSidenet").onclick = async () => {
  renderResult(await callOp("sidenet_view", {}));
};
document.getElementById("btnPair").onclick = async () => {
  renderResult(await callOp("pair_status", {}));
};
window.fraggate = fraggate;
document.getElementById("stage").innerHTML = homePanel();
paintTabs();
callOp("pair_status", {}).then(setPairBadge).catch(() => {});
